...
public class BasicBehaviour2 : MonoBehaviour
{
	...
	private List<GenericBehaviourP2> behaviours;            // The list containing all the enabled player behaviours.
	private List<GenericBehaviourP2> overridingBehaviours;  // List of current overriding behaviours.
	...
	void Awake ()
	{
		...
		behaviours = new List<GenericBehaviourP2> ();
		overridingBehaviours = new List<GenericBehaviourP2>();
	...
	void Update()
	{
		...
		h = Input.GetAxis("moveh");
		v = Input.GetAxis("movev");
	...
	void FixedUpdate()
	{
		...
		{
			foreach (GenericBehaviourP2 behaviour in behaviours)
			{
		...
		{
			foreach (GenericBehaviourP2 behaviour in overridingBehaviours)
			{
				behaviour.LocalFixedUpdate();
			}
		}
	...
	public void SubscribeBehaviour(GenericBehaviourP2 behaviour)
	{
	...
	public bool OverrideWithBehaviour(GenericBehaviourP2 behaviour)
	{
		...
		if (!overridingBehaviours.Contains(behaviour))
		{
			...
			if (overridingBehaviours.Count == 0)
			{
				...
				foreach (GenericBehaviourP2 overriddenBehaviour in behaviours)
				{
	...
	public bool RevokeOverridingBehaviour(GenericBehaviourP2 behaviour)
	{
		if (overridingBehaviours.Contains(behaviour))
		{
	...
	public bool IsOverriding(GenericBehaviourP2 behaviour = null)
	{
		if (behaviour == null)
...
public abstract class GenericBehaviourP2 : MonoBehaviour
{
	//protected Animator anim;                       // Reference to the Animator component.
	protected int speedFloat;                      // Speed parameter on the Animator.
	protected BasicBehaviour2 behaviourManager;     // Reference to the basic behaviour manager.
	...
	{
		// Set up the references.
		behaviourManager = GetComponent<BasicBehaviour2> ();
